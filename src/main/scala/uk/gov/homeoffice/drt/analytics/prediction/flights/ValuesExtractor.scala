...
case class ValuesExtractor(extraction: (
    UtcDate,
    Terminal
) => Future[Map[WithId, Iterable[(Double, Seq[String], Seq[Double], String)]]])(implicit ec: ExecutionContext) {
  private val log = LoggerFactory.getLogger(getClass)

  val extractValuesByKey
      : (Terminal, SDateLike, Int) => Source[(WithId, Iterable[(Double, Seq[String], Seq[Double], String)]), NotUsed] =
  ...
  private def accumulate(
      acc: Map[WithId, Iterable[(Double, Seq[String], Seq[Double], String)]],
  ...
  private def extractValuesForDate(terminal: Terminal, date: UtcDate)(implicit
      ec: ExecutionContext
