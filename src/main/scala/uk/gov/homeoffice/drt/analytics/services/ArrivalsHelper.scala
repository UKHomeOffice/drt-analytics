...
object ArrivalsHelper {
  ...
  def populateMaxPax()(implicit
      ...
      ec: ExecutionContext,
      timeout: Timeout
  ): (UtcDate, Iterable[Arrival]) => Future[Iterable[Arrival]] =
    (date, arrivals) => {
      ...
      if (arrivals.nonEmpty && pctOk < 95) {
        ...
        arrivalsActor
          ...
          .map { a =>
            ...
            log.info(
              s"Populated max pax for $date, was $pctOk%, now $pctOk2% of ${arrivals.size} arrivals have max pax"
            )
            a
          }
          .recover {
