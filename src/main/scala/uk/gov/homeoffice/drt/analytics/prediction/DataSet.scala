...
case class DataSet(df: DataFrame, features: List[Feature[_]]) {
  ...
  val featuresWithOneToManyValues: FeaturesWithOneToManyValues =
    FeaturesWithOneToManyValues(features, oneToManyFeatureValues)

  def trainModel(labelCol: String, trainingSplitPercentage: Int)(implicit
  ...
  def evaluate(labelCol: String, trainingSplitPercentage: Int, model: LinearRegressionModel)(implicit
      session: SparkSession
  ): LinearRegressionSummary =
    model.evaluate(prepareDataFrame(labelCol, trainingSplitPercentage, sortAscending = true))

  def predict(labelCol: String, predictionSplitPercentage: Int, model: LinearRegressionModel)(implicit
      session: SparkSession
  ): DataFrame =
    model
  ...
  private def prepareDataFrame(labelColName: String, takePercentage: Int, sortAscending: Boolean)(implicit
      session: SparkSession
  ): DataFrame = {
