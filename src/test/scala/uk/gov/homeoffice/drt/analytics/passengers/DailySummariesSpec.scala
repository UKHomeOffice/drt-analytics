...
object MockArrivalsActor {
  def props: Arrivals => (String, SDateLike) => Props =
    (arrivals: Arrivals) => (_: String, _: SDateLike) => Props(new MockArrivalsActor(arrivals))
}
...
class DailySummariesSpec extends TestKit(ActorSystem("passengers-actor")) with SpecificationLike {
  ...
  val forecastArrival: SimpleArrival = SimpleArrival(
    "BA",
    1,
  ...
  val liveArrival: SimpleArrival = SimpleArrival(
    "BA",
    1,
    date.millisSinceEpoch,
    "T1",
  ...
  "Given a sourcePersistenceId, and props for a mock actor with an arrival" >> {
    ...
    "When I ask for arrivals from sources" >> {
      ...
      val eventualsArrivals = DailySummaries.arrivalsForSources(
        List(forecastPid),
        date.toUtcDate,
        date.toUtcDate,
        props(genArrivals(Seq(forecastArrival)))
      )
  ...
  "Given a live and a forecast version of the same arrival" >> {
    "When I ask to merge live and forecast arrivals" >> {
      ...
      val mergedArrivals =
        Await.result(DailySummaries.mergeArrivals(Seq(futureForecastArrivals, futureLiveArrivals)), 1.second)
  ...
  "Given one arrival" >> {
    "When I ask for the summary csv line for the two days starting from scheduled date of the arrival for" >> {
      ...
      "I should get a csv line showing the date, terminal, origin and pax count for that date and a '-' for the day after" >> {
        val eventualCountsByOrigin = DailySummaries.dailyPaxCountsForDayByOrigin(
          date.toUtcDate,
          date.toUtcDate,
          numberOfDays,
          "T1",
          Future(genArrivals(Seq(liveArrival)).arrivals)
        )
    ...
    "When I ask for the daily summary for that 1 day when the arrival is scheduled for" >> {
      "I should get a map of the arrival's origin to a DailyPaxCountOnDay containing 1 day with the pax from the 1 arrival" >> {
        val summaries = Await.result(
          DailySummaries.dailyPaxCountsForDayByOrigin(
            date.toUtcDate,
            date.toUtcDate,
            1,
            "T1",
        ...
        val summaries = Await
          .result(
            DailySummaries.dailyPaxCountsForDayByOrigin(
              ...
              1,
              "T1",
              Future(genArrivals(Seq(liveArrival, arrival2)).arrivals)
            ),
            1.second
          )

        summaries === Map(liveArrival.origin -> DailyPaxCountsOnDay(
          date.toUtcDate,
          Map(date.millisSinceEpoch ->
            (liveArrival.bestPaxEstimate.getPcpPax.getOrElse(0) +
  ...
  "Given 2 arrivals scheduled for the same day, each with 100 pax and 10 transit pax" >> {
    "When I ask for the daily summary for the day when the arrival is scheduled for" >> {
      "I should get a map of the arrival's origin to a DailyPaxCountOnDay containing 1 day with the total pax from the 2 arrivals minus the transit pax" >> {
        ...
        val summaries = Await.result(
          DailySummaries
            .dailyPaxCountsForDayByOrigin(
              date.toUtcDate,
              date.toUtcDate,
              1,
              "T1",
              Future(genArrivals(Seq(arrival, arrival2)).arrivals)
            ),
          1.second
        )

        summaries === Map(liveArrival.origin ->
          DailyPaxCountsOnDay(date.toUtcDate, Map(date.millisSinceEpoch -> (100 * 2 - 10 * 2))))
      }
    }
  }

  private def genArrivals(arrivals: Seq[SimpleArrival]): Arrivals =
    Arrivals(arrivals.map(a => (a.uniqueArrival, a)).toMap)
}
